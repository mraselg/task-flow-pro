...
import { CATEGORY_LABELS, MOCK_MEMBERS, type TaskCategory } from "@/lib/data";
...
const AddTaskModal = ({ open, onClose }: AddTaskModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Mock submit - will connect to backend later
    alert("Task created! (Demo mode)");
    onClose();
    setTitle("");
    setDescription("");
    setDeadline("");
  ...
  return (
    <div className="fixed inset-0 bg-foreground/40 backdrop-blur-sm z-[60] flex items-end lg:items-center justify-center" onClick={onClose}>
      <div
        className="bg-card rounded-t-2xl lg:rounded-2xl w-full lg:max-w-lg max-h-[85vh] overflow-y-auto animate-slide-up"
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              required
              className="w-full px-4 py-2.5 rounded-lg bg-muted border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-2.5 rounded-lg bg-muted border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring resize-none"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 rounded-lg bg-muted border border-border text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              >
          ...
          <div>
            ...
            <input
              ...
              required
              className="w-full px-4 py-2.5 rounded-lg bg-muted border border-border text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
            />
          </div>

...
import { Clock, CalendarDays, ChevronRight, Search, Filter, X, AlertTriangle, Loader2 } from "lucide-react";
...
const TasksPage = ({ searchQuery = "", initialFilter }: TasksPageProps) => {
  ...
  useEffect(() => {
    if (initialFilter && initialFilter !== "all") {
      setActiveStatus(initialFilter as TaskStatus);
    }
  }, [initialFilter]);
  ...
  }, [tasks, members, activeStatus, selectedCategory, selectedPriority, effectiveSearch]);
      ...
      <div className="flex gap-2 mb-4">
        ...
        <button
          ...
          className={`px-4 py-2.5 rounded-xl border text-sm font-medium flex items-center gap-2 transition-colors ${
            showFilters || activeFilters > 0 ? "bg-secondary/10 text-secondary border-secondary/30" : "bg-muted border-border text-muted-foreground hover:bg-muted/80"
          }`}
        >
          <Filter className="w-4 h-4" />
          <span className="hidden sm:inline">Filter</span>
          {activeFilters > 0 && (
            <span className="w-5 h-5 rounded-full bg-secondary text-secondary-foreground text-[10px] font-bold flex items-center justify-center">{activeFilters}</span>
          )}
        </button>
      </div>
          ...
          <div className="flex flex-wrap gap-4">
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                <button onClick={() => setSelectedCategory("all")} className={`text-xs px-3 py-1.5 rounded-lg font-medium transition-colors ${selectedCategory === "all" ? "bg-secondary/15 text-secondary" : "bg-muted text-muted-foreground"}`}>All</button>
                {(["web_design", "digital_marketing", "graphic_video"] as TaskCategory[]).map(cat => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)} className={`text-xs px-3 py-1.5 rounded-lg font-medium transition-colors ${selectedCategory === cat ? "bg-secondary/15 text-secondary" : "bg-muted text-muted-foreground"}`}>{CATEGORY_LABELS[cat]}</button>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-1.5">
                {["all", "high", "medium", "low"].map(p => (
                  <button key={p} onClick={() => setSelectedPriority(p)} className={`text-xs px-3 py-1.5 rounded-lg font-medium capitalize transition-colors ${selectedPriority === p ? "bg-secondary/15 text-secondary" : "bg-muted text-muted-foreground"}`}>{p}</button>
                ))}
              </div>
            </div>
          </div>
          {activeFilters > 0 && (
            <button onClick={() => { setSelectedCategory("all"); setSelectedPriority("all"); }} className="mt-3 text-xs text-destructive font-medium hover:underline">Clear all filters</button>
          )}
        </div>
      )}
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              {tab.label}
              <span className={`text-xs px-1.5 py-0.5 rounded-md ${activeStatus === tab.id ? `${tab.color.split(" ")[0]}/25` : "bg-background"}`}>{count}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="space-y-3">
        {filteredTasks.length === 0 ? (
          <div className="glass-card rounded-xl p-12 text-center">
            <p className="text-muted-foreground text-sm">{effectiveSearch ? `No tasks matching "${effectiveSearch}"` : "No tasks in this category"}</p>
          </div>
        ) : (
          filteredTasks.map((task) => {
            ...
            return (
              <div key={task.id} onClick={() => setSelectedTask(task)} className="glass-card rounded-xl p-4 lg:p-5 hover:shadow-md transition-all cursor-pointer group">
                  ...
                  <div className="flex gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        ...
                        {task.status !== "completed" && daysLeft !== null && daysLeft <= 3 && daysLeft >= 0 && (
                      ...
                      <div className="flex items-center gap-4 mt-3 text-xs text-muted-foreground">
                        ...
                        {task.deadline && <span className="flex items-center gap-1"><CalendarDays className="w-3 h-3" />{task.deadline}</span>}
                      </div>
                    </div>
                  </div>
